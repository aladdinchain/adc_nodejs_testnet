...
var bigi = require('bigi'),
    ...
    Signature = require('./auth/signature'),
    KeyPrivate = require('./auth/key_private'),
    PublicKey = require('./auth/key_public'),
...
Auth.generateAddress = function (name, prefix) {
	var pubKeys = {};
	var seed = name + "address" + prefix;
	var brainKey = seed.trim().split(/[\t\n\v\f\r ]+/).join(' ');
	var hashSha256 = hash.sha256(brainKey);
	var bigInt = bigi.fromBuffer(hashSha256);
	var toPubKey = secp256k1.G.multiply(bigInt);
	...
	pubKeys = prefix + bs58.encode(addy);
	return pubKeys;
};
...
/**
	...
	@arg {array} roles - defaults to standard Futurepia blockchain-level roles
*/
Auth.getPrivateKeys = function (name, password) {
	var roles = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ['owner', 'active', 'memo'];
...
Auth.isWif = function (privWif) {
	...
	try {
		...
		var bufWif = Buffer.from(bs58.decode(privWif));
...
Auth.signTransaction = function (trx, keys) {
	...
	var cid = Buffer.from(config.chain_id, 'hex');
